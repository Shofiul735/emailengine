...
async function init(args) {
    ...
    server.route({
        ...
        options: {
            ...
            notes: ['Registers new IMAP account to be synced', BEHAVIOR.CREDENTIALS_VERIFIED_LATER],
            tags: ['api', 'Account'],

            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.CREDENTIALS_VERIFIED_LATER],
    ...
    server.route({
        ...
        async handler(request) {
            ...
            try {
                ...
                // Credentials in this payload came from an API client, so a credential change here
                // can mean the account was re-authorized - the only caller allowed to arm the
                // reconnect gate in update().
                return await accountObject.update(request.payload, { reauthorized: true });
            } catch (err) {
                handleError(request, err);
            }
        },
        options: {
            ...
            notes: ['Updates account information', BEHAVIOR.CREDENTIALS_VERIFIED_LATER],
            tags: ['api', 'Account'],

            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.CREDENTIALS_VERIFIED_LATER],
                    responses: apiResponses('Returns the account ID. Changed credentials take effect on the next connection.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            notes: ['Requests connection to be reconnected', BEHAVIOR.SCHEDULED_ON_WORKER],
            tags: ['api', 'Account'],

            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.SCHEDULED_ON_WORKER],
                    responses: apiResponses(
                        'Returns an acknowledgement. The reconnect is requested, not completed - watch the account state for the result.',
                        400,
                        401,
                        403,
                        404,
    ...
    server.route({
        ...
        options: {
            ...
            notes: ['Immediately trigger account syncing for IMAP accounts', BEHAVIOR.SCHEDULED_ON_WORKER],
            tags: ['api', 'Account'],
            ...
            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.SCHEDULED_ON_WORKER],
                    responses: apiResponses(
                        'Returns an acknowledgement. A full mailbox sync is scheduled and runs in the background.',
                        400,
                        401,
                        403,
                        404,
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.DELETION_SCOPE],
                    responses: apiResponses(
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses('Returns a page of accounts with their connection states.', 400, 401, 403, 429, 500)
                }
            },
            ...
            validate: {
                ...
                query: Joi.object({
                    ...
                    state: Joi.string()
                        ...
                        .meta({ enumDescriptions: ENUM_DESCRIPTIONS.accountState })
                        .label('AccountState'),
    ...
    server.route({
        ...
        options: {
            ...
            notes: ['Returns stored information about the account. Passwords are not included.', BEHAVIOR.NO_QUOTA_ON_API_ACCOUNTS],
            tags: ['api', 'Account'],

            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.NO_QUOTA_ON_API_ACCOUNTS],
                    responses: apiResponses('Returns the account configuration, connection state and sync status.', 400, 401, 403, 404, 429, 500)
                }
            },
            ...
            response: {
                schema: Joi.object({
                    ...
                    outlookSubscription: Joi.object({
                        ...
                        state: Joi.object({
                            state: Joi.string()
                                .valid('creating', 'created', 'renewing', 'error')
                                .description('Subscription state')
                                .meta({ enumDescriptions: ENUM_DESCRIPTIONS.pubsubState }),
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses(
                        'Returns a valid OAuth2 access token for the account, refreshing it first if it had expired.',
                        400,
                        401,
                        403,
                        404,
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses('Returns the signatures configured on the account, as reported by the provider.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses('Returns a single-use URL for the hosted authentication form.', 400, 401, 403, 404, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            notes: ['Output is a downloadable text file', BEHAVIOR.IMAP_ONLY],
            tags: ['api', 'Logs'],
            ...
            plugins: {
                openapi: {
                    'x-ee-behavior': [BEHAVIOR.IMAP_ONLY],
                    produces: ['text/plain'],
                    responses: apiResponses('Returns the account log as a plain text stream.', 400, 401, 403, 429, 500)
                }
            },
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses(
                        'Returns the outcome of the IMAP and SMTP connection tests, including per-protocol errors when a test failed.',
                        400,
                        401,
                        403,
                        429,
    ...
    server.route({
        ...
        options: {
            ...
            plugins: {
                openapi: {
                    responses: apiResponses(
                        'Returns the discovered IMAP and SMTP settings for the address, or empty values when nothing could be resolved.',
                        400,
                        401,
                        403,
                        429,
