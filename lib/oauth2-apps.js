...
class OAuth2AppsHandler {
    ...
    // without touching account data. The source settings keys are deleted after a successful
    // copy, which is what makes the migration idempotent; a Redis lock guards against parallel
    // instances sharing one Redis. Returns the list of migrated app IDs.
    async migrateLegacyApps() {
        ...
        try {
            ...
            for (let legacyKey of LEGACY_KEYS) {
                ...
                // the copy (or cleanup) is complete, drop the source settings keys (no-op when unset)
                let clearReq = this.redis.multi();
                for (let field of FIELDS) {
                    clearReq = clearReq.hdel(this.getSettingsKey(), `${legacyKey}${field}`);
                }
                await clearReq.exec();
            }
            ...
            if (migrated.length) {
                let migratedSet = new Set(migrated);
                ...
                for (let i = 0; i < accounts.length; i += CHUNK_SIZE) {
                    ...
                    for (let j = 0; j < chunk.length; j++) {
                        ...
                        if (oauth2Data && oauth2Data.provider && migratedSet.has(oauth2Data.provider)) {
