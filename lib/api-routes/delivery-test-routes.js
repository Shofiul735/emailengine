...
const { accountIdSchema, apiResponses } = require('../schemas');
...
<p>${now}</p>`,
        ...
        options: {
            ...
            notes: ['Initiate a delivery test', BEHAVIOR.BACKGROUND_JOB],
            tags: ['api', 'Delivery Test'],

            plugins: {
                openapi: {
                    'x-ee-impact': IMPACT.SENDS,
                    'x-ee-behavior': [BEHAVIOR.BACKGROUND_JOB],
                    responses: apiResponses(
                        'Returns the ID of the started deliverability test. Poll the check endpoint for the result.',
                        400,
                        401,
                        403,
                        404,
    ...
    server.route({
        ...
        async handler(request) {
            try {
                request.logger.debug({ msg: 'Requested SMTP delivery test check', deliveryTest: request.params.deliveryTest });
        ...
        options: {
            ...
            plugins: {
                mcp: {
                    name: 'check_delivery_test',
                    title: 'Check delivery test',
                    description:
                        'Check the result of a delivery test started over the REST API: whether the probe message arrived, and its SPF, DKIM and DMARC results.'
                },
                openapi: {
                    responses: apiResponses(
                        'Returns the test result once it has completed, including SPF, DKIM and DMARC outcomes.',
                        400,
                        401,
                        403,
                        404,
